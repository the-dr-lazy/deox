...
export const createSnackbar = createAction(
  ...
  resolve => ({
    duration = 3000,
    ...options
  }: SnackbarOptions & { duration?: number }) => resolve(options, { duration })
)
...
export const dismissSnackbarCompleted = createAction(
  '[Snackbar] dismiss completed'
)
  ...
  action$.pipe(
    ...
    switchMap(([action, state]) => {
      ...
      return fromRoot.getSnackbar(state)
        ? action$.pipe(
            ofType(dismissSnackbarCompleted),
            take(1),
            mapTo(showAction)
          )
        : of(showAction)
...
export const dismissSnackbarEpic = (
  action$: Observable<Actions>,
  _: any,
  { scheduler = async }
) =>
  action$.pipe(
    ...
    delayWhen(({ meta }) =>
      merge(
        action$.pipe(
          ofType(showSnackbarCompleted),
          take(1),
          delay(meta.duration, scheduler)
        ),
        action$.pipe(
          ofType(createSnackbar),
          take(1)

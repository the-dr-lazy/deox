...

{
    ...
    const addItem = createActionCreator('ADD_ITEM', resolve => (name: string) => resolve(name))
    const removeItem = createActionCreator('REMOVE_ITEM', resolve => (name: string) => resolve(name))

    // @dts-jest:pass:snap mutable items list scenario
    createReducer(<ItemsState>[], handleAction => [
        handleAction(identity, state => state),
        handleAction(addItem, (state, { payload }) => [...state, { name: payload }]),
        handleAction(removeItem, (state, { payload }) => state.filter(({ name }) => name !== payload)),
    ])
}
...

{
    ...
    // @dts-jest:pass:snap counter scenario
    createReducer(0, handleAction => [handleAction(increment, state => state + 1), handleAction.others(() => 0)])
}

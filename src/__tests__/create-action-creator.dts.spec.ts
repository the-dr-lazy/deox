...
// @dts-jest:pass:snap
createActionCreator('C', resolve => (error: Error, meta?: { status: number }) => resolve(error, meta))
...
createActionCreator(
    'E',
    // tslint:disable-next-line:no-inferrable-types
    resolve => (name: string, completed: boolean = false) => resolve({ name, completed }, 'Meta'),
)
...
// @dts-jest:pass:snap
createActionCreator('G', resolve => <TName, TMeta>(name: TName, meta: TMeta) => resolve(name, meta))

...
describe('createActionCreator', () => {
    ...
    it('should create action creator with type-only action with empty executor', () => {
        const actionCreator = createActionCreator('[Todo] truncate', resolve => () => resolve())

        expect(actionCreator()).toMatchSnapshot()
    })

    it('should create action creator with error payload', () => {
        const actionCreator = createActionCreator('[Todo] fetch rejected', resolve => (error: Error) => resolve(error))

        expect(actionCreator(new Error())).toMatchSnapshot()
    })

    it('should create action creator with error payload and meta', () => {
        const actionCreator = createActionCreator('[Todo] fetch rejected', resolve => (error: Error, meta: { status: number }) =>
            resolve(error, meta),
        )
    ...
    it('should create action creator with non-error payload', () => {
        const actionCreator = createActionCreator(
            ...
            resolve => (name: string, completed: boolean = false) => resolve({ name, completed }),
        )

        expect(actionCreator('Prepare for practical parasitology course!')).toMatchSnapshot()
    })

    it('should create action creator with non-error payload and meta', () => {
        const actionCreator = createActionCreator('[Todo] add', resolve => (name: string, meta: string) => resolve({ name }, meta))

        expect(actionCreator('Prepare for practical parasitology course!', 'Be patient')).toMatchSnapshot()
    })
})

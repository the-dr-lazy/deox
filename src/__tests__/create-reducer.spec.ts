...
describe('createReducer', () => {
    describe('no default handler', () => {
        ...
        const handleReset = jest.fn((_: number, { payload }: ReturnType<typeof reset>) => payload)
        ...
        it('should initiate with default state when state is undefined', () => {
            expect(counterReducer(undefined, increment())).toBe(counterReducer(defaultCounterState, increment()))
        })

        it('should pass through state when there is no proper handler', () => {
            expect(counterReducer(defaultCounterState, { type: 'NOT DEFINED' })).toBe(defaultCounterState)
        })
    ...
    describe('default handler', () => {
        ...
        const drawNumber = createActionCreator('[Paint] draw number', resolve => (payload: number) => resolve(payload))

        const defaultCounterState = 0

        const handleReset = jest.fn(() => defaultCounterState)
        const handleOthers = jest.fn((state: number, { payload }: Action<string, number>) => (payload % 2 === 0 ? state + 1 : state))
        ...
        it('should initiate with default state when state is undefined', () => {
            expect(evenNumbersCounterReducer(undefined, drawNumber(4))).toBe(evenNumbersCounterReducer(defaultCounterState, drawNumber(4)))
        })
        ...
        it('should calls the default handler when there is no proper handler', () => {
            ...
            expect(evenNumbersCounterReducer(defaultCounterState, action)).toBe(handleOthers(defaultCounterState, action))
        })
    })
})

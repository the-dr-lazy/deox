...
export function createActionCreator<TType extends string, TCallable extends <_T>(...args: any[]) => Action<TType>>(
    type: TType,
    executor: Executor<TType, TCallable>,
): ExactActionCreator<TType, TCallable>
export function createActionCreator<TType extends string, TCallable extends <_T>(...args: any[]) => Action<TType>>(
    type: TType,
    executor: Executor<TType, TCallable> = resolve => (() => resolve()) as TCallable,
) {
    const callable = executor((payload, meta) => createAction(type, payload!, meta!))
